...
#define COPY_ROOT_PATH(buf, name) strncpy(buf, "./", 2); strncat(buf, name, strlen(name)); strncat(buf, "/", 1);
    ...
    loadingbar_new = (
        ...
        "                 * bar coords to on-screen bottom-center default. Gated\n"
        "                 * on s == &loadingbg[0] (model-cache slot only) to\n"
        "                 * avoid two simultaneous bars on PAKs that author BOTH\n"
        "                 * loadingbg + loadingbg2 off-screen (Avengers, PDC2). */\n"
        "                if (s == &loadingbg[0] &&\n"
        "                    (pos_x < 0 || pos_y < 0 ||\n"
