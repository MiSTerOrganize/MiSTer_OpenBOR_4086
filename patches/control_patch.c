 ...
 *   bit 8  = Xbox LB (Left Bumper)  -> FLAG_ATTACK3 (Attack3 -- e.g. DD style switch)
 *   bit 9  = Xbox RB (Right Bumper) -> FLAG_ATTACK4 (Attack4)
 *   bit 10 = Start                  -> FLAG_START
 *
 * Attack3/Attack4 added 2026-07 (mirrors OpenBOR_7533) so OpenBOR games using
 * all 6 action buttons are fully playable. OpenBOR's standard layout puts
 * Attack3/Attack4 on the shoulder bumpers -- Xbox LB and Xbox RB (the CONF_STR
 * jn L/R slots). CONF_STR gains Attack3,Attack4 before Start (Start shifts
 * bit 8 -> bit 10; RBF + binary ship together). The FPGA already writes the
 * full 32-bit joystick word to DDR3.
 *
...
void control_update(s_playercontrols ** playercontrols, int numplayers)
{
    ...
    for(player = 0; player < numplayers; player++)
    {
...
#ifdef MISTER_NATIVE_VIDEO
        {
            ...
            if (joy & 0x100) k |= FLAG_ATTACK3;    /* Xbox LB (Left Bumper)  = Attack3 */
            if (joy & 0x200) k |= FLAG_ATTACK4;    /* Xbox RB (Right Bumper) = Attack4 */
