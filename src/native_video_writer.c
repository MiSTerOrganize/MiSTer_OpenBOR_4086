...
void NativeVideoWriter_WriteFrame(const void* pixels, int width, int height,
    ...
    /* DIAG: Continuous frame capture for palette debug. Mirror of 7533
     * for cross-build comparison. Writes engine's native frame as
     * RGB888 PPM every 120 frames (2s @ 60fps), up to 600 captures =
     * 20 min coverage. Per user request 2026-05-18 for A Tale of
     * Vengeance palette bug. TEMPORARY — revert after fix. */
    {
