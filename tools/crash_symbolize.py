#!/usr/bin/env python3
"""
crash_symbolize.py -- turn an OpenBOR_4086 SIGSEGV dump into "crashed in
...
The engine's mister_crash_handler (patches/sdlport_patch.c) prints:
    ...
    Maps (first 5):
      <base>-<end> r-xp <off> ... /media/fat/games/OpenBOR/OpenBOR_4086
    === END CRASH ===
...
def parse_dump(text, binpath):
    ...
    for m in re.finditer(r"([0-9a-f]+)-[0-9a-f]+ ..x. ([0-9a-f]+).*?(\S+)$", text, re.M):
        ...
        if path.endswith(bn) or "OpenBOR" in path:
